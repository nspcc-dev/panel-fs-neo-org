...
export default function EACLPanel({
	...
	const onSetEACL = (containerId) => {
		if (eACLParams.every((eACLItem) => eACLItem.operation !== '' && eACLItem.action !== '' && eACLItem.targets[0].role !== '' && (!eACLItem.targets[0].accounts || eACLItem.targets[0].accounts?.length > 0) && eACLItem.filters.every((filterItem) => filterItem.headerType !== '' && filterItem.matchType !== '' && filterItem.key !== '' && filterItem.value !== ''))) {
			setError({ active: false, type: [], text: '' });
			setLoadingForm(true);
			...
			{eACLParams.map((eACLItem, index) => (
				...
				>
					...
					>
						...
						{`Rule #${index + 1} ${eACLItem.operation && eACLItem.action && eACLItem.targets[0].role ? `(${eACLItem.operation}-${eACLItem.action}-${eACLItem.targets[0].role === 'USER' ? `${eACLItem.targets[0].accounts ? 'ACCOUNTS' : 'USER'}` : eACLItem.targets[0].role})` : ''}`}
						...
						<div className="panel-block-content">
							<Form.Field kind="group">
								...
								<Form.Control>
									<Form.Select
										renderAs="select"
										value={eACLItem.targets[0].role === 'USER' ? `${eACLItem.targets[0].accounts ? 'ACCOUNTS' : 'USER'}` : eACLItem.targets[0].role}
										className={isError.active && isError.type.indexOf('eacl') !== -1 && eACLItem.targets[0].role.length === 0 ? 'is-error' : ""}
										onChange={(e) => {
											const aECLParamsTemp = [...eACLParams];
											aECLParamsTemp[index].targets = [e.target.value === 'ACCOUNTS' ? { accounts: [], role: "USER" } : { accounts: null, role: e.target.value }];
											setEACLParams(aECLParamsTemp);
										}}
										disabled={!isEdit || isLoadingForm}
									>
